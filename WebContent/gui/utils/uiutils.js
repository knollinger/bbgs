var UIUtils = (function() {
	...
	removeChildsWithClass : function(parent, className) {
	    ...
	    while (allChilds.length) {
		parent.removeChild(allChilds[0]);
	    }
	},
	...
	removeClass : function(element, clazz) {
		...
		if (Array.isArray(element)) {
		    UIUtils.removeClassForMultipleElements(element, clazz);
		} else {
		    var node = UIUtils.getElement(element);
	...
	/**
	 * Entferne die angegebene CSS-Klasse von mehreren Elementen
	 * 
	 * @param elements
	 *                wenn ein String angegeben ist, so wird dieser als
	 *                Element-ID betrachtet. Anderenfalls wird element als
	 ...
	 *                die CSS-Klasse
	 */
	removeClassForMultipleElements : function(elements, clazz) {

	    if (elements) {
		for (var i = 0; i < elements.length; i++) {
		    UIUtils.removeClass(elements[i], clazz);
		}
	    }
	},
	...
	addClass : function(element, clazz) {
		...
		if (Array.isArray(element)) {
		    UIUtils.addClassForMultipleElements(element, clazz)
		} else {
		    var node = UIUtils.getElement(element);
		    var classes = UIUtils.getClasses(node);
	 ...
	 *                die CSS-Klasse
	 */
	addClassForMultipleElements : function(elements, clazz) {

	    if (elements) {
		for (var i = 0; i < elements.length; i++) {
		    UIUtils.addClass(elements[i], clazz);
		}
	    }
	},
	...
	/**
	 * Finde das Parent-Element (up to document) mit dem gegebenen TagName
	 */
	findParentOfType : function(elem, type) {

	    var child = UIUtils.getElement(elem).parentElement;
	    while (child.parentElement != document) {
