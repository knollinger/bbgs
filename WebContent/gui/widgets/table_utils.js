...
TableDecorator.prototype.sortByCol = function(col, colNr) {
	...
	var col1 = self.getColValue(row1.getElementsByTagName("td")[colNr]);
	var col2 = self.getColValue(row2.getElementsByTagName("td")[colNr]);
        if (col1 > col2) {
            return direction;
        }
        if (col2 > col1) {
...
/**
 * Liefere den Inhalt einer Zelle. Wenn die Zelle ein HTMLChild hat, wird dessen
 * Value geliefert. Anderenfalls einfach der TextContent der Zelle
 */
TableDecorator.prototype.getColValue = function(col) {
    
    if(col.children.length) {
	return col.children[0].value;
    }
...
TableDecorator.prototype.createTableSearcherIcon = function() {
    ...
    var cell = this.hdrCols[0];
    var self = this;
    UIUtils.addClass(cell, "search-icon");
    cell.addEventListener("click", function(evt) {
        evt.stopPropagation();
        self.showTableSearcher(cell);
    });
}
 ...
 */
TableDecorator.prototype.showTableSearcher = function(cell) {

    var glasspane = this.createGlasspane();
    var edit = this.createSearchInput();
    glasspane.appendChild(edit);

    var iconRect = cell.getBoundingClientRect();
    var bodyRect = document.body.getBoundingClientRect();
    edit.className = "search-field";
    edit.style.left = iconRect.right;
    edit.style.top = iconRect.top;
